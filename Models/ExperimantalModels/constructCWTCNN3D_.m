function lgraph = constructCWTCNN3D_(parameters)
    spatialDim = 64;
    ...
    for k = 1:parameters.numBlocks
        layers = [
            convolution3dLayer([min(parameters.filterSize, ceil(spatialDim/4^(k-1))) parameters.filterSize parameters.numChannels], parameters.numFilters/2^(k-1),...
            ...
            averagePooling3dLayer([2, 1, 1], Name="avg" + k, Stride=[2, 1, 1])
    ...
    outputName = lgraph.Layers(end).Name;
    layers = [%globalAveragePooling2dLayer(Name='avg_layer')
            flattenLayer(Name="avg_layer")
