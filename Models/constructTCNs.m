function dlnet = constructTCNs(parameters)

    layer  = sequenceInputLayer(parameters.numChannels, Name="sequenceInputLayer"); % Normalization removed 2023.07
    ...
    for k = 1:parameters.numBlocks
        ...
        layers = [
            convolution1dLayer(parameters.filterSize, parameters.numFilters, DilationFactor=dilationFactor, Padding="causal", Name="conv1_" + k)
            layerNormalizationLayer
            spatialDropoutLayer(parameters.dropoutFactor)
            convolution1dLayer(parameters.filterSize, parameters.numFilters, DilationFactor=dilationFactor, Padding="causal")
            layerNormalizationLayer
            %reluLayer
            swishLayer
            spatialDropoutLayer(parameters.dropoutFactor)
